...
def test_address_modify_street():
    ...
    assert(test_address.get_street() = new_test_street), \
...
def test_address_get_apt():
    ...
    assert(test_address.get_apartment() = TEST_APT_NO), \
        "get_apartment() did not return apartment no. line for address"

def test_address_get_apt_none():
    test_address = address.Address(TEST_STREET, TEST_CITY, TEST_STATE_CODE, TEST_ZIP_CODE, \
        TEST_TYPE_STRING)
    assert(test_address.get_apartment() = None), \
        "get_apartment() did not return None for nonexistant apartment no. line for address"

def test_address_modify_apt():
    test_address = address.Address(TEST_STREET, TEST_CITY, TEST_STATE_CODE, TEST_ZIP_CODE, \
        TEST_TYPE_STRING)
    test_address.modify_apartment(TEST_APT_NO)
    assert(test_address.get_apartment() = TEST_APT_NO), \
        "modify_apartment() did not modify and return apartment no. line for address"
...
def test_address_get_zip():
    test_address = address.Address(TEST_STREET, TEST_CITY, TEST_STATE_CODE, TEST_ZIP_CODE, \
        TEST_TYPE_STRING)
    assert(test_address.get_zip() = TEST_ZIPCODE), \
        "get_zip() did not return zipcode for address"

def test_address_modify_zip():
    new_zipcode = 99999
    test_address = address.Address(TEST_STREET, TEST_CITY, TEST_STATE_CODE, TEST_ZIP_CODE, \
        TEST_TYPE_STRING)
    test_address.modify_zip(new_zipcode)
    assert(test_address.get_zip() = new_zipcode), \
        "modify_zip() did not modify and return zipcode for address"
...
def test_address_modify_zip_invalid_type():
    ...
    test_address.modify_zip(new_zipcode)
    assert(test_address.get_zip() = TEST_ZIPCODE), \
        "modify_zip() did not appropriatly handle invalid zip code for address"
...
def test_address_modify_zip_invalid_number():
    ...
    test_address.modify_zip(new_zipcode)
    assert(test_address.get_zip() = TEST_ZIPCODE), \
        "modify_zip() did not appropriatly handle invalid numeric zip code for address"
...
def test_address_get_type():
    test_address = address.Address(TEST_STREET, TEST_CITY, TEST_STATE_CODE, TEST_ZIP_CODE, \
        TEST_TYPE)
    assert(test_address.get_state() = TEST_TYPE), \
...
def test_address_modify_type_to_warehouse():
    ...
    assert(test_address.get_state() = new_type_string), \
        "modify_type() did not modify and return type for change to %s type address" \
        %(new_type_string)

def test_address_modify_type_to_business():
    new_type_string = "business"
    test_address = address.Address(TEST_STREET, TEST_CITY, TEST_STATE_CODE, TEST_ZIP_CODE, \
        TEST_TYPE)
    test_address.modify_type(new_type_string)
    assert(test_address.get_state() = new_type_string), \
