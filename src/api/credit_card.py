...
class CreditCard():
    ...
    def __init__(self, given_id):
        ...
        if isinstance(given_id, int):
            # Ensuring warehouse ID is in credit card table
    ...
    @staticmethod
    def new_creditcard(person_reference, card_number, security_code, expiration_month, \
        expiration_year, card_type, address_reference):
        # Adding new warehouse to database, and returning reference to that warehouse
        db = database.connect()
        cursor = database.get_cursor(db)

        # Ensuring address is in table
        cursor.execute("select id from addresses where id = :input_id", \
            input_id = address_reference.get_id())
        address_id = cursor.fetchone()
        if address_id:
            address_id = address_id[0]
        else:
            raise ValueError("Given address is not in address table")

        # Ensuring person is in table
        ...
        if person_id:
            ...
            raise ValueError("Given person is not in person table")
